...
print(fit_multi_NB, pars = c('alpha', 'beta', 'beta_super','phi'), probs=c(0.05,0.95))
```
...
standata_hier <- standata_simple
standata_hier$M <- standata_hier_orig$M
standata_hier$mo_idx <- standata_hier_orig$mo_idx
standata_hier$J <- standata_hier_orig$J
standata_hier$K <- standata_hier_orig$K
standata_hier$building_idx <- standata_hier_orig$building_idx
standata_hier$building_data <- standata_hier_orig$building_data
standata_hier$mub_sigma_mean <- 0
standata_hier$mub_sigma_sd <- 1
standata_hier$zeta_mean <- 0
standata_hier$zeta_sd <- 1
```
...
```{r fit_hier_NB-low-ESS}
print(fit_hier_NB, pars = c('alpha', 'beta', 'mub','mub_sigma','zeta','phi'), probs=c(0.05,0.95))
```
...
mcmc_trace(
  ...
  as.array(fit_hier_NB,pars = 'mub_sigma'),
...
prior_draws <- cbind("mu" = mu_prior, "log(mub_sigma)" = log(sigma_mu_prior))
...
parcoord_with_divs <- mcmc_parcoord(
  as.array(fit_hier_NB, pars = c("mub_sigma", "mu")),
  np = nuts_params(fit_hier_NB)
)
parcoord_with_divs
```
...
# use the optional probs argument to set the credible interval you want
print(fit_hier_NB, pars = c('alpha', 'beta', 'mub','mub_sigma','zeta','phi'), probs=c(0.05,0.95))
```
...
```{r ppc-full-hier}
y_rep <- as.matrix(fit_hier_NB_ncp, pars = "pred_complaints_rep")
