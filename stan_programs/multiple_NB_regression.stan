...
transformed parameters {
  ...
  vector[N] mu = alpha + beta * traps + log_sq_foot + beta_super * live_in_super;
}
...
generated quantities {
  ...
  for (n in 1:N) {
    real mu_rep = alpha + beta * traps[n] + log_sq_foot[n] + beta_super * live_in_super[n];
    if (mu_rep>20) {
      reject("mu out of bounds, rejected...");
    }
    pred_complaints_rep[n] = neg_binomial_2_log_rng(mu_rep, phi);
  }
}
